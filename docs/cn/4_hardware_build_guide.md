...
| 3 | V5 智能电机 (11W) | 276-4840 | 2 | **绿色齿轮箱 (18:1, 200RPM)** 已预装 |
| 4 | V5 惯性传感器 (IMU) | 276-4855 | 1 | 陀螺仪 + 加速度计 |
| 5 | V5 智能线缆 (300mm) | 276-4850 | 2 | 连接电机 |
| 6 | V5 智能线缆 (600mm) | 276-4851 | 1 | 连接 IMU（需要较长线缆） |
| 7 | V5 遥控器 | 276-4820 | 1 | 无线或 USB 接线模式 |
| 8 | V5 电池充电器 | 276-4812 | 1 | 比赛间隙充电 |
| 9 | USB-C 数据线 | — | 1 | 上传程序到主控 |
...
|---|------|------|-------------|------|------|
| 1 | 4英寸全向轮 | 直径 101.6 mm | 276-3526 | 2 | 主驱动轮 — 滚轮设计允许转弯时侧滑 |
| 2 | 高强度 (HS) 轴 — 4" | 长度 101.6mm，1/4" 方孔 | 276-2293 | 2 | 驱动轴 |
| 3 | 高强度 (HS) 轴 — 2" | 长度 50.8mm，1/4" 方孔 | 276-2296 | 2 | **（方案A — 齿轮传动）** 电机中间轴 |
| 4 | 60齿高强度齿轮 | 模数0.8，HS孔 | 276-2169 | 2 | **（方案A — 齿轮传动）** 被动齿轮，安装在驱动轴上 |
| 5 | 36齿高强度齿轮 | 模数0.8，HS孔 | 276-2167 | 2 | **（方案A — 齿轮传动）** 主动齿轮，安装在电机轴上 |
| 6 | HS 电机轴适配器 | 将电机输出适配到 HS 轴 | 276-8532 | 2 | 无论齿轮传动还是直驱都需要 |
| 7 | 标准轴 — 2" | 长度 50.8mm，标准 1/8" 方孔 | 276-2011 | 1 | 支撑轮轴 |
...
| 测量项 | 测量方法 | 典型值 | 配置常量 |
|--------|---------|--------|---------|
| **轮子直径** | 用游标卡尺测量橡胶胎面外缘到外缘，穿过圆心 | 101.6mm → 0.1016m | `WHEEL_DIAMETER` |
| **轮距** | 左轮胎接地面中心 → 右轮胎接地面中心 | ~254-381mm → 0.254-0.381m | `WHEEL_TRACK` |
| **每转脉冲数** | 绿色=360，红色=900，蓝色=1800；如有外部齿轮需乘以齿轮比 | 360（直驱）或 600（60:36） | `TICKS_PER_REV` |
...
> **如果你使用不同端口：** 更新 `include/config.h` 中的三个常量。

---

## 5. 软件配置

组装完成后，用实际测量值更新 `include/config.h`：

```cpp
// ── 用游标卡尺测量！ ──
constexpr double WHEEL_DIAMETER   = 0.1016;   // 4" 全向轮 = 0.1016m
constexpr double WHEEL_TRACK      = 0.280;    // 请测量你的机器人！（米）
constexpr double TICKS_PER_REV    = 360.0;    // 360 直驱
                                               // 600 如果使用 60:36 齿轮
...
// 如果使用红色齿轮箱：ratio36_1   （同时 TICKS_PER_REV = 900）
// 如果使用蓝色齿轮箱：ratio6_1    （同时 TICKS_PER_REV = 1800）
```
...
1. 在 `main()` 循环中添加调试代码（在 `odometry_update()` 之后）：
   ...
   ```
2. 手动转动轮子一整圈 → 直驱应显示 ~360（±5），60:36 齿轮应显示 ~600（±5）
...
| 直线行驶冲过目标 | `MAX_VELOCITY` 太高 | 在 config.h 中减小 `MAX_VELOCITY` |
| `make` 编译失败 | SDK 路径错误 | 确保 VEX 扩展已安装且 SDK 路径存在 |
...
║                                                                      ║
║  结构件                                                              ║
║    C型铝材 1×2×1×25                     276-2288       ×2           ║
║    C型铝材 1×2×1×15                     276-2286       ×2           ║
║    角码 1×1                             276-1066       ×4           ║
║                                                                      ║
║  传动系统（直驱）                                                     ║
║    4" 全向轮                            276-3526       ×2           ║
...
║                                                                      ║
║  传动系统（齿轮传动 — 额外添加）                                       ║
║    60T HS 齿轮                          276-2169       ×2           ║
║    36T HS 齿轮                          276-2167       ×2           ║
║    HS 轴 2"                             276-2296       ×2           ║
║                                                                      ║
║  支撑轮                                                              ║
║    2.75" 全向轮                         276-3525       ×1           ║
...
║                                                                      ║
║  紧固件                                                              ║
║    8-32 × 0.250" 星形螺丝              276-6690       ~30          ║
║    8-32 × 0.375" 星形螺丝              276-6691       ~10          ║
║    8-32 × 0.500" 星形螺丝              276-6692       ~20          ║
║    8-32 × 0.750" 星形螺丝              276-6694       ~8           ║
║    8-32 × 1.000" 星形螺丝              276-6696       ~4           ║
║    锯齿螺母 (Keps) 8-32                276-4796       ~40          ║
║    尼龙锁紧螺母 (Nylock) 8-32           276-4800       ~30          ║
║                                                                      ║
║  垫片与轴环                                                          ║
║    薄垫片 1/16"                         276-1212       ~12          ║
║    尼龙垫片 1/8"                        276-1200       ~20          ║
║    尼龙垫片 1/4"                        276-1201       ~10          ║
║    HS 垫片 1/8"                         276-2312       ~12          ║
║    HS 垫片 1/4"                         276-2313       ~8           ║
║    标准轴环                             276-2010       ×4           ║
║    HS 锁紧轴环                          276-2308       ×4           ║
║                                                                      ║
║  铜柱                                                                ║
║    1" 铜柱 8-32                         276-2010       ×4           ║
║                                                                      ║
║  其他                                                                ║
║    扎带（小号）                          —              ~15          ║
║                                                                      ║
