...
> **本指南涵盖 6电机差速驱动 + 垂直双追踪轮 + IMU + AI 视觉传感器的单一硬件配置。** 所有常量直接定义在 `config.h` 中 — 无需 `#ifdef` 切换。
...
| # | 零件 | VEX 零件编号 | 数量 | 备注 |
|---|------|-------------|------|------|
| 1 | V5 主控 (Brain) | 276-4810 | 1 | 主控制器 |
| 2 | V5 电池 | 276-4811 | 1 | 锂离子电池，使用前充电 >50% |
| 3 | V5 智能电机 (11W) | 276-4840 | 6 | 蓝色齿轮箱 (6:1, 600RPM)。左侧：端口 1,2,3（反转）；右侧：端口 4,5,6 |
| 4 | V5 惯性传感器 (IMU) | 276-4855 | 1 | 陀螺仪 + 加速度计，端口 10 |
| 5 | V5 旋转传感器 | 276-4856 | 2 | 纵向追踪轮（端口 8）+ 横向追踪轮（端口 9） |
| 6 | V5 AI 视觉传感器 | 276-4857 | 1 | 端口 12，目标检测 + 颜色识别 |
| 7 | V5 智能线缆 (300mm) | 276-4850 | 6 | 连接电机 |
| 8 | V5 智能线缆 (600mm) | 276-4851 | 4 | 连接 IMU、追踪轮、AI 视觉（需要较长线缆） |
| 9 | V5 遥控器 | 276-4820 | 1 | 无线或 USB 接线模式 |
| 10 | V5 电池充电器 | 276-4812 | 1 | 比赛间隙充电 |
| 11 | USB-C 数据线 | — | 1 | 上传程序到主控 |
...
|---|------|------|-------------|------|------|
| 1 | 3.25英寸全向轮 | 直径 82.55 mm | 276-3524 | 6 | 所有 6 个驱动轮 |
| 2 | 2.75英寸全向轮 | 直径 69.85 mm | 276-3525 | 2 | 追踪轮（纵向 + 横向） |
| 3 | 高强度 (HS) 轴 — 4" | 长度 101.6mm，1/4" 方孔 | 276-2293 | 6 | 驱动轴（每侧 3 根） |
| 4 | HS 电机轴适配器 | 将电机输出适配到 HS 轴 | 276-8532 | 6 | 直驱 — 电机直接连接驱动轴 |

> **本方案使用直驱**（蓝色齿轮箱 6:1, 600 RPM）— 无外部齿轮。每个电机通过 HS 电机轴适配器直接连接到驱动轴。
...
| 7 | 标准轴环 | 固定螺丝锁紧 | 适配 1/8" 轴 | 276-2010 | 4 | 锁定标准轴（追踪轮） |
...
| 2 | 标准平面轴承 | 压入梁孔，标准孔径 | 276-1209 | 2 | 支撑追踪轮的标准轴 |
...
两个垂直安装的追踪轮（死轮）提供高精度位置感知。它们是无动力的 2.75 英寸全向轮，配备 V5 旋转传感器。

| # | 零件 | 规格 | VEX 零件编号 | 数量 | 备注 |
|---|------|------|-------------|------|------|
| 1 | 2.75英寸全向轮 | 直径 69.85mm | 276-3525 | 2 | 纵向追踪轮 + 横向追踪轮 |
| 2 | V5 旋转传感器 | 36000 脉冲/转 | 276-4856 | 2 | 每个追踪轮一个 |
| 3 | 标准轴 — 2" | 长度 50.8mm，1/8" 方孔 | 276-2011 | 2 | 追踪轮轴 |
| 4 | 标准轴环 | 固定螺丝锁紧 | 276-2010 | 4 | 锁定追踪轴 |
| 5 | 标准平面轴承 | 标准孔径 | 276-1209 | 4 | 支撑追踪轴 |
| 6 | 尼龙垫片 1/8" | 标准孔径 | 276-1200 | 4 | 使轮子在轴上居中 |

> **垂直双轮布局（十字形）：** 纵向追踪轮与机器人前进方向一致，测量前后位移。横向追踪轮与其垂直，测量左右位移。旋转角度 100% 由 IMU 提供。
...
| 测量项 | 测量方法 | 值 | 配置常量 |
|--------|---------|---|----------|
| **轮子直径** | 用游标卡尺测量橡胶胎面外缘到外缘，穿过圆心 | 82.55mm → 0.08255m | `WHEEL_DIAMETER` |
| **轮距** | 左轮胎接地面中心 → 右轮胎接地面中心 | ~330mm → 0.330m | `WHEEL_TRACK` |
| **每转脉冲数** | 蓝色齿轮箱 (6:1)，直驱 | 300 | `TICKS_PER_REV` |
| **追踪轮直径** | 用游标卡尺测量 2.75" 全向轮胎面 | 69.85mm → 0.06985m | `TRACKING_WHEEL_DIAMETER` |
...
**目标：** 六根高强度轴穿过底盘，安装 3.25 英寸全向轮（每侧 3 个）。
...
4. 将 **3.25" 全向轮** 安装到每根轴的外端 — 将方轴压入轮子的方孔中
...
**目标：** 两个垂直安装的无动力追踪轮，配备 V5 旋转传感器，用于高精度里程计。
...
1. **纵向追踪轮：** 将一个 2.75" 全向轮安装在标准轴上，与机器人前进方向一致（机器人前后移动时滚动）。将 V5 旋转传感器连接到轴上。使用弹簧浮动支架安装在机器人中心附近，确保轮子始终贴地。
2. **横向追踪轮：** 将第二个 2.75" 全向轮安装在标准轴上，垂直于纵向轮（机器人左右侧滑时滚动）。连接其 V5 旋转传感器。使用弹簧浮动支架安装在机器人中心附近。
3. 用 **标准轴环** 在两侧锁定每根轴。
4. 纵向追踪轮传感器连接到 **端口 8**，横向追踪轮连接到 **端口 9**。

**质量检查：** 两个追踪轮自由旋转并始终紧贴地面。前推机器人时只有纵向追踪轮转动。侧推机器人时只有横向追踪轮转动。
...
| 设备 | 主控端口 | 线缆长度 | 配置常量 |
|------|---------|---------|----------|
| 左前电机 | **端口 1** | 300mm | `LEFT_FRONT_MOTOR_PORT = 0` |
| 左中电机 | **端口 2** | 300mm | `LEFT_MID_MOTOR_PORT = 1` |
| 左后电机 | **端口 3** | 300mm | `LEFT_REAR_MOTOR_PORT = 2` |
| 右前电机 | **端口 4** | 300mm | `RIGHT_FRONT_MOTOR_PORT = 3` |
| 右中电机 | **端口 5** | 300mm | `RIGHT_MID_MOTOR_PORT = 4` |
| 右后电机 | **端口 6** | 300mm | `RIGHT_REAR_MOTOR_PORT = 5` |
| 纵向追踪轮 | **端口 8** | 600mm | `FORWARD_TRACKING_PORT = 7` |
| 横向追踪轮 | **端口 9** | 600mm | `LATERAL_TRACKING_PORT = 8` |
| 惯性传感器 (IMU) | **端口 10** | 600mm | `IMU_PORT = 9` |
| AI 视觉传感器 | **端口 12** | 600mm | （在视觉模块中定义） |
...
> **如果你使用不同端口：** 更新 `include/config.h` 中的端口常量。

---

## 5. 软件配置

组装完成后，验证 `include/config.h` 中的物理常量与实际测量值匹配：

> **注意：** `config.h` 是一个平面文件，没有 `#ifdef` 或 `#define` 切换。直接编辑常量即可。
...
// 电机端口（VEX SDK 从 0 开始：物理端口 1 = 索引 0）
constexpr int LEFT_FRONT_MOTOR_PORT  = 0;   // 端口 1
constexpr int LEFT_MID_MOTOR_PORT    = 1;   // 端口 2
constexpr int LEFT_REAR_MOTOR_PORT   = 2;   // 端口 3
constexpr int RIGHT_FRONT_MOTOR_PORT = 3;   // 端口 4
constexpr int RIGHT_MID_MOTOR_PORT   = 4;   // 端口 5
constexpr int RIGHT_REAR_MOTOR_PORT  = 5;   // 端口 6

constexpr int IMU_PORT = 9;                  // 端口 10

// 追踪轮（垂直十字布局）
constexpr int  FORWARD_TRACKING_PORT = 7;    // 端口 8 — 纵向追踪轮
constexpr int  LATERAL_TRACKING_PORT = 8;    // 端口 9 — 横向追踪轮
constexpr double TRACKING_WHEEL_DIAMETER = 0.06985; // 2.75" 全向轮
```

**`src/main.cpp` 中的电机初始化：**

```cpp
// 6电机差速驱动 — 蓝色齿轮箱 (6:1, 600 RPM)
motor LeftFront  = motor(LEFT_FRONT_MOTOR_PORT,  ratio6_1, true);   // 左侧反转
...
2. 前推左摇杆 → 左轮（3个）向前转
3. 前推右摇杆 → 右轮（3个）向前转
4. 验证每侧 3 个轮子以相同速度同步旋转

- **失败？** 在 `main.cpp` 中翻转该电机的 `true`↔`false`
- **仍然有问题？** 检查左侧电机是否设置了 `reversed=true`，右侧是否为 `reversed=false`
...
| PID积分延迟 | 积分饱和 | 检查 INTEGRAL_LIMIT 值 || `make` 编译失败 | SDK 路径错误 | 确保 VEX 扩展已安装且 SDK 路径存在 |
...
║  V5 惯性传感器                         276-4855       ×1             ║
║  V5 旋转传感器                         276-4856       ×2             ║
║  V5 AI 视觉传感器                      276-4857       ×1             ║
║  智能线缆 300mm                       276-4850       ×6             ║
║  智能线缆 600mm                       276-4851       ×4             ║
║  3.25" 全向轮（驱动）                  276-3524       ×6             ║
║  2.75" 全向轮（追踪）                  276-3525       ×2             ║
║  HS 轴 4"                             276-2293       ×6             ║
...
> **通用零件：** V5 主控、V5 电池、V5 遥控器、V5 电池充电器、USB-C 线、C型铝材、横梁、角码、紧固件、垫片。详见物料清单第1节。
