...
> **配置说明：** 以下流程首先展示 2电机（ROBOT_2MOTOR）版本。当 6电机（ROBOT_6MOTOR）配置有差异时，会提供 6电机变体。测试前确保 `config.h` 中激活了正确的 `#define`。
...
**如果方向错误：** 检查 `main.cpp` 中电机构造函数的反转标志。对于 6电机配置，左侧电机应设置 `reversed=true`，右侧电机设置 `reversed=false`。同时验证所有 6 个电机端口与 `config.h` 一致。

**通过标准：**
- **2电机：** 向前推摇杆时两轮都向前转。
- **6电机：** 所有 6 个轮子向前转。左侧和右侧组同步运动。
...
| 配置 | 齿轮箱 | 预期每转脉冲数 | 编码器来源 |
|------|-------|----------------|----------|
| ROBOT_2MOTOR | 绿色 (18:1) | 360 ±5 | 每个驱动电机直接读取 |
| ROBOT_6MOTOR | 蓝色 (6:1) | 300 ±5 | 中间电机 (`ENCODER_MOTOR_INDEX=1`) |

**6电机说明：** 编码器读数来自每侧的单个电机（中间电机，索引 1）。测试里程计时确保中间电机的轮子与地面良好接触。如果编码器值异常，请验证 `config.h` 中的 `ENCODER_MOTOR_INDEX` 和 `TICKS_PER_REV`。
...
- [ ] **配置：** `config.h` 中激活了正确的机器人配置（`ROBOT_2MOTOR` 或 `ROBOT_6MOTOR`）
- [ ] **电池：** 充满电（> 95%）
- [ ] **固件：** V5主控固件已更新
- [ ] **代码编译：** `make` 构建无错误
- [ ] **单元测试通过：** `make test` → 26/26
- [ ] **IMU校准：** 3秒内屏幕显示 "Ready."
- [ ] **电机检查：** 所有电机方向正确（测试2.1）
...
- [ ] **超时有效：** 抓住机器人时停止（测试4.3）
- [ ] **手动控制：** 坦克驱动平稳
- [ ] **没有松动的线：** 所有连接牢固（2电机配置需 2 根线缆，6电机配置需 6 根线缆）
...
| 配置 | TURN_KP | TURN_KI | TURN_KD | 说明 |
|------|---------|---------|---------|------|
| ROBOT_2MOTOR | 2.0 | 0.0 | 0.1 | 保守值，适合入门 |
| ROBOT_6MOTOR | 3.5 | 0.02 | 0.25 | 更激进，适合 3× 扭矩 |
...
| 配置 | DRIVE_KP | DRIVE_KI | DRIVE_KD | MAX_VELOCITY | MAX_ACCEL |
|------|----------|----------|----------|-----------|-----------|
...
| 11 | 配置错误 | `#define ROBOT_2MOTOR` 与 `ROBOT_6MOTOR` 不匹配 | 检查 `config.h` — 只能取消注释一个 |
| 12 | 6电机：一侧较弱 | 某个电机线缆未完全插入 | 重新插入所有 6 根线缆；逐个检查电机 |
| 13 | 6电机：编码器跳变 | 中间电机轮子未接触地面 | 确保编码器电机（索引 1）有良好附着力 |
| 14 | 回旋镖路径太宽/太窄 | BOOMERANG_LEAD 不合适 | 增大（更窄）或减小（更宽）BOOMERANG_LEAD |
| 15 | PID积分饱和后反应迟钝 | 未启用抗饱和 | 6电机已自动启用；检查 INTEGRAL_LIMIT 值 |
...
║  每转脉冲：   360           300                ║
║  电机数：      2             6                  ║
║  轮子：       4" (0.1016m)  3.25" (0.083m)    ║
║  最大速度：    0.8 m/s       1.2 m/s            ║
║  转弯超时：    2000 ms       1500 ms            ║
║  行驶超时：    5000 ms       4000 ms            ║
║                                              ║
╚══════════════════════════════════════════════╝
```
