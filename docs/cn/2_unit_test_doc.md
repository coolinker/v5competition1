...
> **多配置说明：** 单元测试完全**与配置无关**。无论你使用 `ROBOT_2MOTOR` 还是 `ROBOT_6MOTOR`，同样的 23 个测试验证同样的 PID、MotionProfile 和 Odometry 数学。测试使用模拟 HAL 函数，完全绕过所有配置相关的电机代码。这体现了架构的可复用性 — 算法只需测试一次，即可适用于任何底盘配置。
...
- HAL 函数 — 只是 VEX API 的包装；没什么可单元测试的。6电机配置中的电机数组逻辑通过机器人上的组件测试来验证（见[完整测试文档](3_full_test_doc.md)）。
- `main.cpp` — 竞赛回调函数、配置选择的条件编译和电机声明；通过运行机器人来测试。
- 配置守卫 — `config.h` 中的 `#error` 编译检查在正常构建流程中自动验证。
